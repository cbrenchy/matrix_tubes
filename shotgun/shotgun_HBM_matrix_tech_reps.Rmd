---
title: "shotgun_matrix_HBM_tech_reps"
...
```{r}
tech_rep_jaccard <- read_tsv("shotgun_data_tech_reps_jaccard.txt")
tech_rep_rpca <- read_tsv("shotgun_data_tech_reps_rpca.txt")
tech_rep_unifrac <- read_tsv("shotgun_data_tech_reps_unifrac.txt")
tech_rep_wunifrac <- read_tsv("shotgun_data_tech_reps_weighted_unifrac.txt")
```
...
```{r}
data_text_W <- data.frame(jaccard = c("W=118", "W=107","W=49","W=89"), RPCA = c("W=161","W=140","W=67","W=93"), unifrac = c("W=127","W=114","W=56","W=92"), wunifrac = c("W=205","W=89","W=57","W=108"),x = 1.5, y = c(1,1,1,1), sample_type2 =c("human_feces", "mouse_feces" ,"saliva_before_brush", "saliva_after_brush"))
...
boxplot_rpca <- ggplot(tech_rep_rpca, aes(x = sample1_extraction_protocol, y = value)) +
  ...
  scale_fill_manual(values = c("blue", "steelblue1")) +
  theme_bw() +
  geom_text(data = data_text_W, mapping = aes(x = x, y = 3.5, label = RPCA),
  size =4) +
  theme(axis.title.x = element_blank(),
        legend.position = "none", 
        legend.title = element_blank(),
...
boxplot_wunifrac <- ggplot(tech_rep_wunifrac, aes(x = sample1_extraction_protocol, y = value)) +
  ...
  geom_boxplot(outlier.alpha = 0, aes(fill = sample1_extraction_protocol)) +
  geom_jitter(width = 0.2, size = 0.5) +
  stat_compare_means(method = "wilcox.test",paired = F,vjust=3, hjust=-0.2,size =4) +
  stat_summary(fun = mean, geom = "point", color = "black", fill = "red", shape = 21) +
  ...
  ylim(c(-0.01,3)) +
  scale_fill_manual(values = c("blue", "steelblue1")) +
  geom_text(data = data_text_W, mapping = aes(x = x, y = 1.8, label = wunifrac),
  size =4) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        legend.position = "none", 
...
```{r}
save_plot('shotgun_figure_tech_rep_20x15.png',
          figure_tech_rep,
          base_width = 16,
          base_height = 9)
```
